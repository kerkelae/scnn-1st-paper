...
def compartment_model_simulation(
    bval, bvecs_isft, ads, rds, fs, odfs, device, bten_shape="linear"
):
    ...
    if bten_shape == "linear":
        response = torch.sum(
            ...
            * torch.exp(
                -torch.sum(
                    bval
                    * rf_btens.unsqueeze(0).unsqueeze(1).to(device)
                    * Ds.unsqueeze(2),
                    dim=(3, 4),
                )
            ),
        ...
        response = torch.sum(
            ...
            * torch.exp(
                -torch.sum(
                    bval
                    * rf_btens_pte.unsqueeze(0).unsqueeze(1).to(device)
                    * Ds.unsqueeze(2),
                    dim=(3, 4),
                )
            ),
